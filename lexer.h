...
#include <stdlib.h>
#include <steel/ll.h>
#include <steel/string_manipulation.h>
#include <string.h>
...
void Cxmlp_Error_Fmt(Loc loc, char *format, ...) {
  ...
  printf("%s:" Cell_Fmt ":" Cell_Fmt ": error: ", loc.filepath, loc.row,
         loc.col);
  vprintf(format, args);
  printf("\n");
  va_end(args);
...
#define Cxmlp_Tokens_Push(tokens, token)                                       \
  (Steel_LL_PushBack(tokens, token, sizeof(Token)))
...
void Cxmlp_Token_Print(Token token) {
  printf("%s:" Cell_Fmt ":" Cell_Fmt ": ", token.loc.filepath, token.loc.row,
         token.loc.col);
  switch (token.kind) {
  case TokenKind_TagOpen:
    printf("<\n");
...
static int Cxmlp_IdenParser(int c) { return isalnum(c) || c == '-'; }

Steel_LL *Cxmlp_Tokenize(char *filepath) {
  FILE *file = fopen(filepath, "r");
  if (!file)
    Cxmlp_Error_Fmt(Cxmlp_Loc_Init(filepath, 0, 0), "could not open file '%s'",
                    filepath);
